...
const AddLocation = async () => {
    ...
    try {
        await router.post('locations/store', formData, {
            onSuccess: page => {
                Swal.fire({
                    toast: true,
                    icon: 'success',
...
const updateLocation = async () => {
    ...
    try {
        await router.post('locations/update/' + id.value, formData, {
            onSuccess: (page) => {
                dialogVisible.value = false;
                resetFormData();
                Swal.fire({
...
// delete location methond
const deleteLocation = (location, index) => {
    Swal.fire({
        title: 'Are you sure',
        ...
        if (result.isConfirmed) {
            try {
                router.delete('locations/destroy/' + location.id, {
                    onSuccess: (page) => {
                        this.delete(location, index);
                        Swal.fire({
                            toast: true,
                            icon: "success",
                            position: "top-end",
...
<template>
    <div>
        <h1 class="font-bold text-center font-2xl">Location List</h1>
    </div>
    <section class="  p-3 sm:p-5">
        <!-- dialog for adding location or editing location -->
        <el-dialog v-model="dialogVisible" :name="editMode ? 'Edit location' : 'Add Location'" width="50%"
            :before-close="handleClose">
            <!-- form start -->

            <form @submit.prevent="editMode ? updateLocation() : AddLocation()">
                        ...
                        <tbody>
                            <tr v-for="(location, index) in locations" :key="location.id"
                                ...
                                <td class="px-4 py-3 flex items-center justify-end">
                                    ...
                                    <button :id="`${location.id}-button`" :data-dropdown-toggle="`${location.id}`"
                                    ...
                                    <div :id="`${location.id}`"
                                        ...
                                        <ul class="py-1 text-sm text-gray-700 dark:text-gray-200"
                                            :aria-labelledby="`${location.id}-button`">

                                            <li>
                                                <a href="#" @click="openEditModal(location)"
                                                    class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Edit</a>
                                            </li>
                                        </ul>
                                        <div class="py-1">
                                            <a href="#" @click="deleteLocation(location, index)"
